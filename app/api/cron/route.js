...
async function getCurrentPrice(symbol) {
  ...
  const result = await fetchCandles(massiveSymbol);

  // TEMPORARY: fetchCandles() now returns a plain array on genuine success,
  // or a debug object ({ candles: [], debugError, httpStatus, ... }) on
  // failure/empty data — see the TEMPORARY DEBUG SHAPE note in
  // lib/sources/massive.js. Normalize both shapes here so this keeps
  // working exactly as before once that debug object starts showing up.
